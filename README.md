...
Library System 2PC and Raft Implementations
...
Gateway (Node 1) sends PrepareCheckout to all participants

Each participant validates:
...
Register a user
Publish a book
Checkout using valid user + book
...
❌ Failure Scenarios
1. Invalid User
Use random user ID → GLOBAL_ABORT
2. Invalid Book
Use non-existing book ID → GLOBAL_ABORT
3. Out of Stock
Checkout same book repeatedly → GLOBAL_ABORT
...
Failure scenarios are simulated using Docker stop/start/pause
